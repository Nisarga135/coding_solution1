...
public class Main {
  public static void main(String[] args) throws Exception {
    ...
    ArrayList<Items> goodies_items = new ArrayList<Items>();

    while(sc.hasNextLine())  
    {
      String current[] = sc.nextLine().split(": ");
      goodies_items.add(new Items(current[0], Integer.parseInt(current[1])));
    }
    sc.close();

    Collections.sort(goodies_items, new Comparator<Items>(){
    ...
    fw.write("\ndifference between the highest price and the lowest price is " + min_diff);
	  fw.close();
  }
}